...
private:
    ...
    bool io_killed = false;                    // Flag: KillIO called, I/O threads must not call deferred tasks
    bool quitting = false;                    // Flag: Quit threads

    HANDLE input_thread_active = INVALID_HANDLE_VALUE;        // Handle: Input thread installed (was a bool)
    unsigned int input_thread_id = 0;
    HANDLE input_signal = INVALID_HANDLE_VALUE;                  // Signal for input thread: execute command
    uint32 input_pb, input_dce;        // Command parameters for input thread

    HANDLE output_thread_active = INVALID_HANDLE_VALUE;    // Handle: Output thread installed (was a bool)
    unsigned int output_thread_id = 0;
    HANDLE output_signal = INVALID_HANDLE_VALUE;                // Signal for output thread: execute command
    uint32 output_pb = 0, output_dce = 0;    // Command parameters for output thread

    DCB mode;                            // Terminal configuration

    bool is_serial = false;
    bool is_parallel = false;                            // true if LPTx
    bool is_file = false;                                    // true if FILE
    bool is_tcp_server = false;

    TCHAR output_file_name[256];

    unsigned int tcp_thread();                 // thread that handle tcp connections
    static unsigned int WINAPI tcp_thread_stub(void *arg) { return ((XSERDPort*)arg)->tcp_thread(); }
...
XSERDPort::XSERDPort(LPCTSTR dev, LPCTSTR suffix)
{
    ...
    is_tcp_server = (_tcsncmp(device_name, TEXT("TCP:"), 4) == 0);
    if(is_file) {
        char entry_name[20];
    ...
    if (is_tcp_server) {
        ...
        char arg[128];
        WideCharToMultiByte(CP_ACP, 0, device_name, wcslen(device_name)+1, arg, sizeof(arg), NULL, NULL);
        mPort = atoi(arg+4);
    }

    is_serial = !is_parallel && !is_file && !is_tcp_server;
...
int16 XSERDPort::open(uint16 config)
{
    ...
    if((!is_tcp_server) && (fd == INVALID_HANDLE_VALUE)) {
        goto open_error;
...
/**
 ...
 open a client socket, signal tcp_output, and start receiving and sending data.
 */
unsigned int XSERDPort::tcp_input()
{
    int error_code;

    D(bug(TEXT("XSERDPort::input_func started for device %s\r\n"), device_name));
    auto mServerEvent = WSACreateEvent();
    auto mClientEvent = WSACreateEvent();
    ...
    for (;;) {
        ...
        if (mServerSocket == INVALID_SOCKET) {
            ...
            int ret = bind(mServerSocket, (struct sockaddr*) & sa, sizeof(sa));
            if (ret == SOCKET_ERROR) {
                printf("TCP Bind: error %d\n", WSAGetLastError());
            }
            WSAEventSelect(mServerSocket, mServerEvent, FD_ACCEPT | FD_CLOSE);

            ret = ::listen(mServerSocket, 0); // blocking?!
            if (ret == SOCKET_ERROR) {
        ...
        int nWaitFor = 0;
        HANDLE waitFor[4];
        waitFor[nWaitFor++] = input_signal;
        waitFor[nWaitFor++] = output_signal;
        if (mServerSocket != INVALID_SOCKET)
            waitFor[nWaitFor++] = mServerEvent;
        if (mClientSocket != INVALID_SOCKET)
            waitFor[nWaitFor++] = mClientEvent;
        DWORD event = WSAWaitForMultipleEvents(nWaitFor-2, waitFor+2, FALSE, /*INFINITE*/5, TRUE);
        event += 2;
        if (event == WSA_WAIT_EVENT_0) {
            ...
            WSAEnumNetworkEvents(mServerSocket, mServerEvent, &eventMap);
            if (eventMap.lNetworkEvents & FD_ACCEPT) {
                struct sockaddr_in sa;
                mClientSocket = ::accept(mServerSocket, nullptr, nullptr);
                if (mClientSocket != INVALID_SOCKET) {
                    WSAEventSelect(mClientSocket, mClientEvent, FD_READ | FD_CLOSE);
                }
            }
            // do we need to handle the close event, or can the client do that?
            // or do we have to call ::listen() again?
        } else if (event == WSA_WAIT_EVENT_0 + 3) {
            // client socket
            int nReceived = -1;
            WSANETWORKEVENTS eventMap;
            WSAEnumNetworkEvents(mClientSocket, mClientEvent, &eventMap);
            if (eventMap.lNetworkEvents & FD_READ) {
                dataAvailable = true;
            }
            if ((eventMap.lNetworkEvents & FD_CLOSE) || (nReceived == 0)) { // or if we received 0 bytes in FD_READ
                ::closesocket(mClientSocket);
                mClientSocket = INVALID_SOCKET;
            }
        }

        // ---- if there is anything in the input buffer, or the Mac just queries for data, handle it
        if (receiveData || dataAvailable)
        {
            ...
            int32 actual = -1;
            if (dataAvailable) {
                actual = ::recv(mClientSocket, (char*)buf, length, 0);
            }
            if (actual == -1) {
                error_code = readErr;
            }
            ...
            if (io_killed) {
                ...
                WriteMacInt16(input_pb + ioResult, abortErr);
                ...
                if (actual >= 0) {
                    ...
                    WriteMacInt32(input_pb + ioActCount, 0);
                    WriteMacInt32(input_dt + serdtResult, error_code);
                }

                // Trigger serial interrupt
                D(bug(" triggering serial interrupt\r\n"));
                WriteMacInt32(input_dt + serdtDCE, input_dce);
                read_done = true;
                SetInterruptFlag(INTFLAG_SERIAL);
                TriggerInterrupt();
            }
        ...
        // ---- if there is anything in the output buffer, handle it
        if (sendData) {
            ...
            int32 actual = -1;
            if (mClientSocket != INVALID_SOCKET) {
                actual = ::send(mClientSocket, (char*)buf, length, 0);
            }
            if (actual == -1) {
                error_code = writErr;
            }
            ...
            if (io_killed) {
                ...
                WriteMacInt16(output_pb + ioResult, abortErr);
                ...
                if (actual >= 0) {
                    ...
                    WriteMacInt32(output_pb + ioActCount, 0);
                    WriteMacInt32(output_dt + serdtResult, error_code);
                }

                // Trigger serial interrupt
                D(bug(" triggering serial interrupt\r\n"));
                WriteMacInt32(output_dt + serdtDCE, output_dce);
                write_done = true;
                SetInterruptFlag(INTFLAG_SERIAL);
                TriggerInterrupt();
            }
